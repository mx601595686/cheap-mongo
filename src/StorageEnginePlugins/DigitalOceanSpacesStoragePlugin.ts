...
export = class DigitalOceanSpacesStoragePlugin implements BaseStorageEnginePlugin {
    ...
    async getConnection(): Promise<BaseStorageEngineConnection> {
        ...
        const enableGzip = (process.env.ENABLE_GZIP || '').toLowerCase() === 'false' ? false : true;    //是否开启gzip压缩
        ...
        const connection: BaseStorageEngineConnection = {
            ...
            async checkConnection() {
                const result = await pQueue.add(() => spaces.getBucketVersioning({ Bucket: process.env.SPACE_NAME as string }).promise());
                if (typeof result.Status !== 'string') throw new Error('DigitalOceanSpacesStoragePlugin 连接异常');
            },
            ...
            get(path: string) {
                return pQueue.add(async () => {
                    ...
                    if (result.Body) {
                        ...
                        throw new Error(`没有找到对应的数据：'${path}'`);
                }, { priority: 1 });
            },
